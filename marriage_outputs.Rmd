...
art_incr_start <- 2025
summary_name <- "marriage_rates_adjusted_2005"
cumulative_years <- 10
...
temp <- list(marriage_rates_baseline, marriage_rates_adjusted, marriage_rates_adjusted_2005)
names(temp) <- c("unedited", "adj_2025", "adj_2005")
...
fig1 <- marriage_rates_combined %>% 
  ...
  scale_fill_discrete("Scenario" , labels = c("Adjusted in 2005", "Adjusted in 2025", "Baseline")) + 
  scale_color_discrete("Scenario" , labels = c("Adjusted in 2005", "Adjusted in 2025", "Baseline"))
...
# Plots
## Married adults
``` {r married_adults_adj, echo=TRUE}
fig3 <- marriage_rates_adjusted %>% 
  filter(scenario == "baseline", 
         pitc_reduction_year == 2025, 
         indicator %in% c("MarriedF17to49", "MarriedM17to49"),
         test_reduction %in% c(0)) %>% 
  ...
  theme(axis.text = element_text(size = 11), 
        ...
        legend.title = element_text(size = 11)) +
  scale_y_continuous("Married adults (millions)", labels =(function(l) {round(l/1e6,1)}), expand = c(0, 0)) +
  ggtitle("Married adults - adjusted in 2025")

```

## Married adults
``` {r marriage_prop_adj, echo=TRUE}
fig4 <- marriage_rates_adjusted %>% 
  filter(scenario == "baseline", 
         pitc_reduction_year == 2025, 
         indicator %in% c("prop_married_m", "prop_married_f"),
         test_reduction %in% c(0)) %>%
  ...
  theme(axis.text = element_text(size = 11), 
        ...
        legend.title = element_text(size = 11)) +
  scale_y_continuous("Married proportion of adults", expand = c(0, 0)) +
  ggtitle("Married proportion of adults - adjusted in 2025")

```

# Plots
## Married adults
``` {r married_adults_adj2, echo=TRUE}
fig3b <- marriage_rates_adjusted_2005 %>% 
  filter(scenario == "baseline", 
         pitc_reduction_year == 2025, 
         indicator %in% c("MarriedF17to49", "MarriedM17to49"),
         test_reduction %in% c(0)) %>% 
  ...
  theme(axis.text = element_text(size = 11), 
        ...
        legend.title = element_text(size = 11)) +
  scale_y_continuous("Married adults (millions)", labels =(function(l) {round(l/1e6,1)}), expand = c(0, 0)) +
  ggtitle("Married adults - adjusted in 2005")

```

## Married adults
``` {r marriage_prop_adj2, echo=TRUE}
fig4b <- marriage_rates_adjusted_2005 %>% 
  filter(scenario == "baseline", 
         pitc_reduction_year == 2025, 
         indicator %in% c("prop_married_m", "prop_married_f"),
         test_reduction %in% c(0)) %>%
  ...
  theme(axis.text = element_text(size = 11), 
        ...
        legend.title = element_text(size = 11)) +
  scale_y_continuous("Married proportion of adults", expand = c(0, 0)) +
  ggtitle("Married proportion of adults - adjusted in 2005")
...
combo_fig <- ggarrange(fig1, fig2, fig3, fig4, fig3b, fig4b, nrow = 3, ncol = 2)
ggsave(filename = "comparing_marriage.png", device = "png", width = 25, height = 20, units  = "cm")
```
## condom usage
``` {r condom_usage_baseline, echo=TRUE}
fig5 <- marriage_rates_baseline %>% 
  filter(scenario == "baseline", 
         pitc_reduction_year == 2025, 
  ...
  ggtitle("Percentage of protected sex acts - baseline")
```

## condom usage
``` {r condom_usage_adj, echo=TRUE}
fig6 <- marriage_rates_adjusted %>% 
  filter(scenario == "baseline", 
         pitc_reduction_year == 2025, 
         indicator %in% c("CondomUsage"),
         test_reduction %in% c(0)) %>%
  ...
  theme(axis.text = element_text(size = 11), 
        ...
        legend.title = element_text(size = 11)) +
  scale_y_continuous("Protected sex acts (%)", expand = c(0, 0)) +
  ggtitle("Percentage of protected sex acts - adjusted")
