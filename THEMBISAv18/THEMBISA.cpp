...
void ReadRollout()
{
	...
	file.ignore(255, '\n');
	for (ig = 0; ig < 6; ig++){
		for (iy = 0; iy < ProjectionTerm; iy++){ file >> SelfTestTotals[iy][ig]; }
	}
	file.ignore(255,'\n');
	file.ignore(255,'\n');
	for(iy=0; iy<ProjectionTerm; iy++){
...
void SetFutureRollout()
{
	...
	// HCT assumptions
	for (iy = 31; iy < ProjectionTerm; iy++){
		HCT1stTimeF25[iy] = FutureInterventions.out[CurrSim - 1][0];}
	StartRollout = FutureInterventions.out[CurrSim - 1][2] + 30;
	for (iy = 0; iy < ProjectionTerm; iy++){ HBCTuptake[iy] = 0.0; }
	if (StartRollout < ProjectionTerm){
		for (iy = StartRollout; iy < ProjectionTerm; iy++){
			HBCTuptake[iy] = FutureInterventions.out[CurrSim - 1][3];}
	}

	// ART assumptions
	StartRollout = FutureInterventions.out[CurrSim - 1][7] + 30;
	for (iy = 30; iy < ProjectionTerm; iy++){ EligibleAsymPre500[iy] = 0.0; }
	if (StartRollout < ProjectionTerm){
		for (iy = StartRollout; iy < ProjectionTerm; iy++){
	...
	for (iy = 0; iy < ProjectionTerm; iy++){POC_CD4[iy] = 0.0;}
	if (StartRollout < ProjectionTerm){
		for (iy = StartRollout; iy < ProjectionTerm; iy++){
			POC_CD4[iy] = 1.0;}
	}
	if (StartRollout < FirstRollout){ FirstRollout = StartRollout; }
	for (iy = 30; iy < ProjectionTerm; iy++){ HCT_ARTuptake[iy] = HCT_ARTuptake[29]; }
	if (FirstRollout < ProjectionTerm){
		for (iy = FirstRollout; iy < ProjectionTerm; iy++){
	...
	for (iy = 30; iy < ProjectionTerm; iy++){ PCRbirth[iy] = 0.0; }
	/*if (StartRollout < ProjectionTerm){
		for (iy = StartRollout; iy < ProjectionTerm; iy++){
			PCRbirth[iy] = 1.0;}
	}*/
	...
	/*for (iy = 30; iy < ProjectionTerm; iy++){PrEP_FSW[iy] = 0.0;}
	if (StartRollout < ProjectionTerm){
		for (iy = StartRollout; iy < ProjectionTerm; iy++){
	...
	for (ig = 0; ig < 2; ig++){
		for (iy = 30; iy < ProjectionTerm; iy++){
			PrEP_15[iy][ig] = 0.0;
			PrEP_20[iy][ig] = 0.0;
		}
		if (StartRollout < ProjectionTerm){
...
void SimulateParameters()
{
	...
	if (InclPriors[68][0] == 1 || InclPriors[69][0] == 1 || InclPriors[70][0] == 1 || InclPriors[71][0] == 1 || InclPriors[72][0] == 1){
		...
		for (iy = 32; iy < ProjectionTerm; iy++){
			RateARTstartF[iy] = RateARTstartF[31]; }
		if (PropnalImmART == 1){
			for (iy = 15; iy < ProjectionTerm; iy++){
				HCT_ARTuptake[iy] = RateARTstartF[iy] * RR_ARTstart1stMo;
	...
	if (InclPriors[78][0] == 1 || InclPriors[79][0] == 1 || InclPriors[80][0] == 1 || InclPriors[81][0] == 1 || InclPriors[82][0] == 1){
		...
		for (iy = 32; iy < ProjectionTerm; iy++){ RateARTstartC[iy] = RateARTstartC[31]; }
		if (PropnalImmART == 1){
			for (iy = 15; iy < ProjectionTerm; iy++){
				PaedARTuptake[iy] = RateARTstartC[iy] * RR_ARTstart1stMo;
