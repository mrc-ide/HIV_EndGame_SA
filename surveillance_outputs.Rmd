...
filepaths <- paste0("THEMBISAv18/results/condom_reduction_", c(0), ".csv")
temp <- lapply(filepaths, read.csv)
names(temp) <- c(0)
condom_reduction <- bind_rows(temp, .id = "condom_usage_decrease")

filepaths <- paste0("THEMBISAv18/results/condom_promotion_", c(0), ".csv")
temp <- lapply(filepaths, read.csv)
names(temp) <- c(0)
...
condom_change %>% 
  ...
  filter(scenario == "intervention", 
         pitc_reduction_year == 2025, 
         indicator %in% c("UndiagnosedHIV","TotalART15", "TotalDiagNotOnART"),
         year >= 1990,
         year <= 2100, 
         test_reduction %in% c("None"),
         future_value == 0, 
         future_variability == "condom_reduction") %>% 
  ...
  geom_bar(aes(colour = indicator), show.legend = T, position = "fill", stat = "identity") +  
  scale_x_continuous("", expand = c(0, 0), breaks = seq(2000, 2100, 20)) +
  expand_limits(y=0) + theme_classic() + 
  theme(axis.text = element_text(size = 11), 
        axis.title.y = element_text(size = 11), 
  ...
  scale_fill_brewer("", aesthetics = c("colour", "fill"),
                    palette = "Set1", direction = 1,
                    labels = c("Diagnosed on ART",
                               "Diagnosed not on ART",
                               # "Total PLWH",
                               "Undiagnosed")
                    ) +
  ggtitle("Proportion of PLWH\n(15+ years)") #+ facet_wrap(~test_reduction)
ggsave(filename = "surveillance_figures/distributions_plwh.png", device = "png", width = 16, height = 10,units = "cm")
```
