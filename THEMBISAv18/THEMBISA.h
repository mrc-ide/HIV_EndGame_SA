...
int StartYear = 1985;
int ProjectionTerm = 56; // Note that entering term of 10 will give you results in 1994
int CurrYear;
...
string ProvID = "WC"; // Choose from EC, FS, GT, KZ, LM, MP, NC, NW, WC
...
const int CalibARTtotalsP = 1; // 1 = calibrate to reported total numbers of children on ART
...
double ProvARTmortAdj; // Relative rate of ART mortality in province, compared to national
...
double PCR6month[86]; // propn of HIV+ mothers who get infant PCR tested at 6 months
double PCRuptake6; // propn of HIV+ mothers who get infant PCR tested at 6 months, in current year
...
double NetMigrPos[91][2]; // Net HIV+ migrants by age and sex (age & HIV status at yr end)
double NetMigrNeg[91][2]; // Net HIV- migrants by age and sex (age & HIV status at yr end)
...
double NewPCRdiagBirth;
double NewPCRdiag12mo; 
double NewPCRdiag24mo;
...
double NewPerinatal;
double NewPostnatal;
double NewPostnatal12;
double NewPostnatal24;
double NewPostnatal18; // Limited to MTCT from mothers who were diagnosed positive
...
double ObservedPrevHCT[14]; // HIV prevalence in adults tested for HIV 2004-8, 2010, 2012, 2015-21
double ObservedPrevHCT_P[7]; // HIV prevalence in kids tested for HIV, 2015-21
...
double CSWstudyDetails[30][3];
double MSMstudyDetails[20][4];
...
double ModelPrevHCT[14];
double ModelPrevHCT_P[7];
...
const int ARTdataPoints = 167; // Number of reported ART totals that combine adults and children
const int ARTdataPointsP = 110; // Number of reported ART totals in children
...
const int MCMCdim = 45; // Number of parameters in uncertainty analysis
const int MaxPriors = 133; // Corresponds to the number of rows of data in Priors.txt
...
const int StepSample=500; // # additional simulations at each IMIS step after the first
const int TotalSimulations=104500; // Must be = InitSample + StepSample * (IMISsteps - 1)
...
PostOutputArray NewHIV1st12mo(56);
PostOutputArray NewHIV12to24mo(56);
PostOutputArray NewHIVto18mo(56);
PostOutputArray NewHIVmothersBF(56);
PostOutputArray VertTransmKnownPos(56);
...
OutputByAge FemIncAS(81, 56);
OutputByAge MalePrevAS(91, 56);
OutputByAge FemPrevAS(91, 56);
OutputByAge MaleMortAS(91, 56);
OutputByAge FemMortAS(91, 56);
OutputByAge MaleDiagAS(91, 56);
OutputByAge FemDiagAS(91, 56);
OutputByAge MaleART_AS(91, 56);
OutputByAge FemART_AS(91, 56);
OutputByAge MaleAIDSdeathsAS(92, 56);
OutputByAge FemAIDSdeathsAS(92, 56);
OutputByAge BirthsPosMomsAS(36, 56);
OutputByAge NonAIDSdeathsPosM(91, 56);
OutputByAge NonAIDSdeathsPosF(91, 56);
OutputByAge NonAIDSdeathsNegM(91, 56);
OutputByAge NonAIDSdeathsNegF(91, 56);
OutputByAge NetMigrantsPosM(91, 56);
OutputByAge NetMigrantsPosF(91, 56);
OutputByAge NetMigrantsNegM(91, 56);
