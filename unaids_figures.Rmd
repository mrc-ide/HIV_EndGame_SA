...
fig1a <- test_reduction_only %>% mutate(test_reduction = as.factor(test_reduction)) %>% 
  ...
  mutate(test_reduction = factor(as.character(test_reduction), labels = c("100", "25", "0"), levels = c("100","25", "0"))) %>% 
  ...
  scale_fill_manual(name = "General\nHTS\nreduction", breaks = c(0, 25, 100), values = c(`0` = "black", `25` = "#377eb8", `100` = "#ff7f00"), labels= c("None", "25", "100%"), aesthetics = c("colour", "fill")) + 
  ggtitle("HIV tests amongst adults \n(millions; 15+ years)") 
fig1a
ggsave(filename = "unaids_figures/total_hiv_tests.png", plot = fig1a, device = "png", units = "cm", height = 13, width = 15)
```

Incidence over time at different years

```{r fig1ady, echo=FALSE}
test_reduction_only <- test_reduction_only %>% mutate(pitc_reduction_year = as.factor(pitc_reduction_year))

fig1ady <- test_reduction_only %>% mutate(test_reduction = as.factor(test_reduction)) %>% 
  filter(scenario == "intervention", 
         pitc_reduction_year %in% c(2025, 2050), 
         indicator == "HIVinc15to49",
         year >= 2020, 
         test_reduction %in% c(50)) %>% 
  mutate(test_reduction = factor(as.character(test_reduction), labels = c("50"), levels = c("50"))) %>% 
  ...
  geom_line(data = filter(test_reduction_only, 
              scenario == "intervention",
              pitc_reduction_year == 2035,
              indicator == "HIVinc15to49",
              year >= 2020,
              test_reduction %in% c(0)),
              aes(year, mean, colour = pitc_reduction_year), show.legend = F) +
  geom_ribbon(data = filter(test_reduction_only, 
              scenario == "intervention",
              pitc_reduction_year == 2035,
              indicator == "HIVinc15to49",
              year >= 2020,
              test_reduction %in% c(0)),
              aes(year, ymin = lower_CI, ymax = upper_CI, fill = pitc_reduction_year), show.legend = F, alpha = 0.10) +
  geom_hline(aes(yintercept = 0.001), lty = "dotted") +
  scale_x_continuous("", expand = c(0, 0)) +
  expand_limits(y=0) + theme_classic() + 
  theme(axis.text = element_text(size = 16), 
  ...
  scale_fill_manual(name = "General\nHTS\nreduction", breaks = c(2035, 2025, 2050), values = c(`2035` = "black", `2025` = "#4daf4a", `2050` = "red"), labels= c("", "None", "100%"), aesthetics = c("colour", "fill")) + 
  ggtitle("HIV incidence \n(per 1000; 15-49 years)") 
fig1ady
ggsave(filename = "unaids_figures/50_red_inc_2025_2050.png", plot = fig1ady, device = "png", units = "cm", height = 13, width = 15)
```
...
fig1b <- test_reduction_only %>% mutate(test_reduction = as.factor(test_reduction)) %>% 
  filter(scenario == "intervention", 
         ...
         year >= 1990, 
         test_reduction %in% c(0)) %>% 
  mutate(test_reduction = factor(as.character(test_reduction), labels = c("0"), levels = c("0"))) %>% 
  ggplot(aes(year, mean, group = test_reduction, fill = test_reduction)) +
  geom_ribbon(aes(ymin = lower_CI, ymax = upper_CI, fill = test_reduction), alpha = 0.10, show.legend = F) +
  geom_line(aes(colour = test_reduction), show.legend = F) +
  scale_x_continuous("",expand = c(0, 0)) +
  ...
  scale_fill_manual(name = "General\nHTS\nreduction", breaks = c(0, 100), values = c(`0` = "black", `100` = "red"), labels= c("None", "100%"), aesthetics = c("colour", "fill")) + 
  ggtitle("HIV prevalence (%)\n(15-49 years)")
fig1b
ggsave(filename = "unaids_figures/baseline_prev.png", plot = fig1b, device = "png", units = "cm", height = 13, width = 15)
```
...
fig1c <- test_reduction_only %>% mutate(test_reduction = as.factor(test_reduction)) %>% 
  ...
  theme(axis.text = element_text(size = 16), 
        ...
        aspect.ratio=1, 
        legend.title = element_text(size = 11)) +
  scale_y_continuous("", labels =(function(l) {round(l*1e2,1)}), limits = c(0,1), expand = c(0,0)) +
  scale_fill_manual(name = "General\nHTS\nreduction", breaks = c(0, 25), values = c(`0` = "black", `25` = "red"), labels= c("None", "25%"), aesthetics = c("colour", "fill")) + 
  ggtitle("ART coverage (%)\n(15+ years)")
ggsave("unaids_figures/baseline_art.png", fig1c, device = "png", height = 13, width = 15, units = "cm")
```
...
fig1d <- test_reduction_only %>% mutate(test_reduction = as.factor(test_reduction)) %>% 
  filter(scenario == "intervention", 
         ...
         indicator == "TotalHIVtests",
         year == 2051, 
         test_reduction %in% c(0,50)) %>% 
  mutate(test_reduction = factor(as.character(test_reduction), labels = c("100", "75", "50", "25", "0"), levels = c("100", "75", "50", "25", "0"))) %>% 
  ggplot(aes(year, mean, group = test_reduction, fill = test_reduction)) +
  geom_ribbon(aes(ymin = lower_CI, ymax = upper_CI, fill = test_reduction), alpha = 0.10, show.legend = F) +
  geom_line(aes(colour = test_reduction), show.legend = F) +
  scale_x_continuous("",expand = c(0, 0)) +
  ...
  theme(axis.text = element_text(size = 16), 
        ...
        aspect.ratio=1, 
        legend.title = element_text(size = 11)) +
  scale_y_continuous("", labels =(function(l) {round(l*1e2,1)}),expand = c(0, 0)) +
  scale_fill_manual(name = "General\nHTS\nreduction", breaks = c("baseline", "0", "25", "50", "75", "100"), values = c("0" = "black", "25" = "#377eb8", "50" = "#4daf4a", "75" = "#984ea3",  "100" = "#ff7f00"), labels= c("Status quo", "None", "25%", "50%", "75%", "100%"), aesthetics = c("colour", "fill")) +
  ggtitle("HIV tests per 100 adults\n(15+ years)")
ggsave("unaids_figures/tests_per_adult.png", fig1d, device = "png", units = "cm", height = 12.57, width = 14.96)
```

Infections over time

```{r fig1e, echo=FALSE}
fig1e <- test_reduction_only %>% mutate(test_reduction = as.factor(test_reduction)) %>% 
  filter(scenario == "intervention", 
         pitc_reduction_year == 2025, 
...
fig1e
ggsave(filename = "unaids_figures/baseline_inf.png", plot = fig1e, device = "png", units = "cm", height = 13, width = 15)
```
...
ggsave(filename = "unaids_figures/fig1.pdf", plot = fig1, device = "pdf", width = 20, units = "cm")
ggsave(filename = "unaids_figures/fig1.png", plot = fig1, device = "png", width = 20, units = "cm")
fig1
```
...
fig2a <- test_reduction_only %>% 
  ...
  filter(test_reduction %in% c(50),
         year == 2100, 
         scenario == "intervention", 
         indicator == "HIVinc15to49", 
         pitc_reduction_year %in% c(2025, 2050)) %>% 
  ggplot(aes(pitc_reduction_year, mean, group = pitc_reduction_year, fill = pitc_reduction_year)) + 
  geom_col(aes(color = pitc_reduction_year), position =  position_dodge(width = 0.85), width = 0.85, alpha = 0.75, show.legend = F) +
  geom_errorbar(aes(ymin = lower_CI, ymax = upper_CI), show.legend = F, position =  position_dodge(width = 0.34), width = 0.34, alpha = 0.5) +
  xlab("General HTS reduction (%)") + 
  theme_classic() + 
  scale_y_continuous("", labels = (function(l) {round(l*1e3,1)})) + 
  scale_x_discrete("", labels = c("50% general\nHTS reduced\nfrom 2025", "50% general\nHTS reduced\nfrom 2050")) +
  expand_limits(y = 0) +
  # geom_hline(aes(yintercept = 0.001), lty = "dotted") +
  theme(axis.text = element_text(size = 16), 
        axis.title.y = element_blank(), 
...
fig2a
ggsave("unaids_figures/inc_2100_2025_v_2050.png", plot = fig2a, device = "png", units = "cm", height = 13, width = 15)
```
...
fig2b <- test_reduction_cumulative %>% 
  filter(indicator == "NewAdultHIV", 
         ...
         test_reduction %in% c(25, 50, 75, 100), pitc_reduction_year %in% c(2025)) %>% 
  mutate(pitc_reduction_year = as.factor(pitc_reduction_year), 
         test_reduction = as.factor(test_reduction)) %>% 
  ggplot(aes(test_reduction, mean, group = test_reduction, fill = test_reduction)) +
  geom_col(aes(color = test_reduction), position =  position_dodge(width = 0.85), width = 0.85, alpha = 0.75, show.legend = F) +
  geom_errorbar(aes(ymin = lower_CI, ymax = upper_CI), show.legend = F, position =  position_dodge(width = 0.34), width = 0.34, alpha = 0.5) +
  scale_y_continuous("", 
                     labels = (function(l) {paste(round(l/1e6,1), "million")})) +
  scale_fill_manual(name = "General\nHTS\nreduction", breaks = c("baseline", "0", "25", "50", "75", "100"), values = c("0" = "black", "25" = "#377eb8", "50" = "#4daf4a", "75" = "#984ea3",  "100" = "#ff7f00"), labels= c("Status quo", "None", "25%", "50%", "75%", "100%"), aesthetics = c("colour", "fill")) +
  scale_x_discrete("", labels = (function(l) {paste(l, "%\nreduction")})) + theme_classic() +
...
fig2b
ggsave("unaids_figures/add_inf_2025.png", device="png", units = "cm", width = 15, height = 13)
```

Additional HIV infections when testing reduced at different times- ART change

``` {r fig2b_art, echo = FALSE}

fig2b_art <- cumulative_art_change %>% 
  filter(indicator == "TotalAIDSdeathsadult", 
         scenario == "intervention", 
         test_reduction %in% c(0, 50), pitc_reduction_year %in% c(2025), 
         future_value %in% c(0, 0.065), future_variability == "art_deterioration",
         pitc_reduction_year == 2025) %>% 
  mutate(pitc_reduction_year = as.factor(pitc_reduction_year), 
         test_reduction = as.factor(test_reduction),
         future_value = as.factor(future_value)) %>% 
  ggplot(aes(test_reduction, mean, group = future_value, fill = future_value)) +
  geom_col(aes(color = future_value), position =  position_dodge(width = 0.85), width = 0.85, alpha = 0.75, show.legend = F) +
  geom_errorbar(aes(ymin = lower_CI, ymax = upper_CI), show.legend = F, position =  position_dodge(width = 0.85), width = 0.34, alpha = 0.5) +
  scale_y_continuous("", 
                     labels = (function(l) {paste(round(l/1e6,1),"million")})) +
  scale_color_brewer("ART coverage\nin 2035",palette = "Dark2",direction = 1, labels = c("77% (Baseline)", "65%")) +
  scale_fill_brewer("ART coverage\nin 2035", palette = "Dark2",direction = 1, labels = c("77% (Baseline)", "65%")) +
  scale_x_discrete("",labels = c("Status quo\ngeneral HTS", "50% general\nHTS reduced\nfrom 2025")) + theme_classic() +
  theme(axis.text = element_text(size = 16), 
        axis.title.y = element_text(size = 16), 
        axis.title.x = element_text(size = 16),
        legend.text = element_text(size = 16), 
        ...
        plot.margin = margin(0,0,0,0)) +
  ggtitle("Cumulative AIDS-deaths\n2025-2075\n(millions; 15+ years)")
fig2b_art
ggsave("unaids_figures/add_dth_2025_art_decrease.png", device="png", units = "cm", height = 13, width = 15)
```

``` {r fig2b_con, echo = FALSE}

fig2b_con <- condom_change_cumulative %>% 
  filter(indicator == "TotalAIDSdeathsadult", 
         scenario == "intervention",
         test_reduction %in% c(0, 50), pitc_reduction_year %in% c(2025), 
         future_value %in% c(0, 7), future_variability == "condom_reduction",
         pitc_reduction_year == 2025) %>% 
  mutate(pitc_reduction_year = as.factor(pitc_reduction_year), 
         test_reduction = as.factor(test_reduction),
         future_value = as.factor(future_value)) %>% 
  ...
  scale_y_continuous("", 
                     labels = (function(l) {paste(round(l/1e6,1),"million")})) +
  scale_color_brewer("Condom usage\nin 2035",palette = "Dark2",direction = 1, labels = c("34% (Baseline)", "23%")) +
  scale_fill_brewer("Condom usage\nin 2035", palette = "Dark2",direction = 1, labels = c("34% (Baseline)", "23%")) +
  scale_x_discrete("",labels = c("Status quo\ngeneral HTS", "50% general\nHTS reduced\nfrom 2025")) + theme_classic() +
  theme(axis.text = element_text(size = 16), 
        axis.title.y = element_text(size = 16), 
        axis.title.x = element_text(size = 16),
  ...
  ggtitle("Cumulative AIDS-deaths\n2025-2075\n(millions; 15+ years)")
fig2b_con
ggsave("unaids_figures/add_dth_2025_con_decrease.png", device="png", units = "cm", height = 13, width = 15)
```
...
fig2c <- test_reduction_cumulative %>% 
  ...
  geom_errorbar(aes(ymin = lower_CI, ymax = upper_CI), show.legend = F, position =  position_dodge(width = 0.34), width = 0.34, alpha = 0.5) +
  scale_y_continuous("", 
                     labels = (function(l) {paste(round(l/1e3,1), "thousand")})) +
  scale_fill_manual(name = "General\nHTS\nreduction", breaks = c("baseline", "0", "25", "50", "75", "100"), values = c("0" = "black", "25" = "#377eb8", "50" = "#4daf4a", "75" = "#984ea3",  "100" = "#ff7f00"), labels= c("Status quo", "None", "25%", "50%", "75%", "100%"), aesthetics = c("colour", "fill")) +
  scale_x_discrete("", labels = (function(l) {paste(l, "%\nreduction")})) + theme_classic() +
  theme(axis.text.y = element_text(size = 16),
        axis.text.x = element_text(size = 14),
...
fig2c
ggsave("unaids_figures/add_dth_2025.png", device="png", units = "cm", height = 13, width = 15)
```

```{r absolute_costs, echo=FALSE}
...
test_reduction_cumulative %>% 
  ...
  mutate(pitc_reduction_year = as.factor(pitc_reduction_year), 
         test_reduction = as.factor(test_reduction)) %>% 
  ggplot(aes(indicator, mean, group = indicator, fill = indicator)) +
  geom_col(aes(color = indicator), position =  position_dodge(width = 0.34), width = 0.34, alpha = 0.75, show.legend = F) +
  geom_errorbar(aes(ymin = lower_CI, ymax = upper_CI), show.legend = F, position =  position_dodge(width = 0.17), width = 0.17, alpha = 0.5) +
  scale_y_continuous("", 
                     labels = (function(l) {paste0(round(l/1e6,1)," million")})) +
  scale_color_brewer(" ",palette = "Dark2",direction = -1, labels = c("New infections", "AIDS-deaths")) +
  scale_fill_brewer(" ", palette = "Dark2",direction = -1, labels = c("New infections", "AIDS-deaths")) +
  scale_x_discrete("", labels = c("New infections", "AIDS-deaths")) + theme_classic() +
  theme(axis.text = element_text(size = 18), 
        ...
        legend.title = element_text(size = 11),
        plot.margin = margin(0,0,0,0)) +
  ggtitle("New HIV infections & AIDS-deaths\n2025-2075\n(15+ years)")
ggsave("unaids_figures/cum_inf_death.png", device = "png", units = "cm", height = 13, width = 15)
```
Additional HIV infections when testing reduced at different times

``` {r fig2ba, echo = FALSE}

fig2ba <- test_reduction_cumulative %>% 
  filter(indicator == "TotalAIDSdeathsadult", 
         scenario == "absolute_dif", 
         test_reduction %in% c(50), pitc_reduction_year %in% c(2025, 2050)) %>% 
  mutate(pitc_reduction_year = as.factor(pitc_reduction_year), 
         test_reduction = as.factor(test_reduction)) %>% 
  ggplot(aes(pitc_reduction_year, mean, group = pitc_reduction_year, fill = pitc_reduction_year)) +
  geom_col(aes(color = pitc_reduction_year), position =  position_dodge(width = 0.85), width = 0.85, alpha = 0.75, show.legend = F) +
  ...
  ggtitle("Additional AIDS-deaths\nover 50-years\n(15+ years)")
fig2ba
ggsave("unaids_figures/add_inf_2025vs_2050.png", device="png", units = "cm", width = 15, height = 13)
```
...
fig2ba <- test_reduction_cumulative %>% 
  ...
  ggtitle("Additional AIDS-deaths\n2025-2075\n(15+ years)")
fig2ba
ggsave("unaids_figures/add_dths_2025vs_2050.png", device="png", units = "cm", width = 15, height = 13)
```
...
temp3dx <- bind_rows(temp3c, temp3d, .id = "indicator")

fig3d <- temp3dx %>% 
  ggplot(aes(test_reduction, art_coverage_2035, z = mean/10**6, group = indicator)) + 
  # with_blur(sigma = 0,
            # geom_raster(aes(fill = mean/10**6), interpolate = TRUE)) +
  geom_contour(aes(color = indicator),breaks = 0, show.legend = F) +
  # geom_contour(aes(),breaks = c(0.5, 1, 1.5), color = "grey",show.legend = FALSE) +
  # geom_text_contour(stroke = 0.1, stroke.colour = "white",check_overlap = TRUE, breaks = c(0), skip = 0,label.placer = label_placer_fraction()) +
  geom_hline(aes(yintercept = 0.768), lty = "dotted") +
  annotate(geom = "text", x=50, y=0.768, label="Baseline", size = 16/.pt) + 
  scale_fill_gradient2(" ",
                       low = "blue",
                       mid = "lightyellow",
                       high = "red", 
                       midpoint = 0) +
  scale_color_discrete("", labels = c("No additional infections",
                                      "No additional deaths")) +
  scale_y_continuous(name ="ART coverage in 2035 (%)",
                     labels =(function(l) {round(l*1e2,1)}),
                     limits = c(0.75,1),
                     expand = c(0,0)) +
  scale_x_continuous("General HTS reduction (%)", limits = c(0,100), expand = c(0,0)) + theme_classic() +
  theme(axis.text = element_text(size = 16),
        axis.title.y = element_text(size = 16),
        axis.title.x = element_text(size = 16),
        legend.text = element_text(size = 16),
        ...
        legend.box.spacing = unit(5, "pt"),
        aspect.ratio=1) + 
  ggtitle("No additional HIV infections or AIDS-deaths\n2025-2075")
fig3d
ggsave("unaids_figures/no_add_deaths_art.png", fig3d, device = "png", height = 13, width = 15, units = "cm")
```
...
fig3
ggsave(filename = "figures/fig3.pdf", plot = fig3, device = "pdf", width = 20, height = 17.8, units = "cm")
ggsave(filename = "figures/fig3.png", plot = fig3, device = "png", width = 20, height = 17.8, units = "cm")
```
...
temp4dx <- bind_rows(temp4c, temp4d, .id = "indicator")

fig4d <- temp4dx %>% 
  ggplot(aes(test_reduction, overall_condom_usage, z = mean/10**6, group = indicator)) + 
  # with_blur(sigma = 0,
  #           geom_raster(aes(fill = mean/10**6), interpolate = TRUE)) +
  geom_contour(aes(color = indicator), breaks = 0,show.legend = FALSE) + 
  ...
  #                   label.placer = label_placer_fraction()) +
  geom_hline(aes(yintercept = 33.9), lty = "dotted") +
  annotate(geom = "text", x=50, y=33.9, label="Baseline", parse = TRUE, size = 16/.pt) + 
  scale_fill_gradient2(" ",
                       low = "blue",
                       mid = "lightyellow",
                       high = "red", 
  ...
  ggtitle("No additional HIV infections or AIDS deaths\n2025-2075")
fig4d
ggsave("unaids_figures/no_deaths_con.png", fig4d, device = "png", units = "cm", height = 13, width = 15)
```
...
fig4
ggsave(filename = "figures/fig4.pdf", plot = fig4, device = "pdf", width = 20, height = 17.8, units = "cm")
ggsave(filename = "figures/fig4.png", plot = fig4, device = "png", width = 20, height = 17.8, units = "cm")
```
...
suppfig5a <- art_change_summary %>% 
  ...
  theme(axis.text = element_text(size = 11), 
        ...
        legend.title = element_text(size = 11)) +
  scale_y_continuous("HIV incidence", labels =(function(l) {round(l*1e3,1)}), breaks = c(0, 0.001, 0.002, 0.003, 0.004, 0.005, 0.006, 0.007), expand =c(0,0)) +
  scale_fill_manual(name = "Treatment\nretention", breaks = c("baseline", "0"), values = c("baseline" = "black", "0" = "#377eb8"), labels= c("Baseline", "Increased"), aesthetics = c("colour", "fill")) +
  ggtitle("HIV incidence\n(per 1000; 15-49 years)")
suppfig5a
...
ggsave(filename = "unaids_figures/suppfig5.pdf", plot = suppfig5, device = "pdf", width = 20, units = "cm")
ggsave(filename = "unaids_figures/suppfig5.png", plot = suppfig5, device = "png", width = 20, units = "cm")
suppfig5
```
...
suppfig6a <- art_change_summary %>% 
  ...
  geom_line(data = filter(art_change_summary, 
              ...
              future_value == 0.07,
              future_variability == "art_deterioration"),
            aes(year, mean, lty = scenario), show.legend = F) +
  geom_ribbon(data = filter(art_change_summary, 
              scenario == "baseline",
              pitc_reduction_year == 2025,
              indicator == "HIVinc15to49",
  ...
  theme(axis.text = element_text(size = 16), 
        ...
        aspect.ratio=1, 
        legend.title = element_text(size = 11)) +
  scale_y_continuous("HIV incidence", labels =(function(l) {round(l*1e3,1)}),expand = c(0,0)) +
  scale_fill_manual(name = "General\nHTS\nreduction", breaks = c("baseline", "0", "25", "50", "75", "100"), values = c("baseline" = "black", "0" = "#ff7f00", "25" = "#377eb8", "50" = "#4daf4a", "75" = "#984ea3",  "100" = "#377eb8"), labels= c("Status quo", "None", "25%", "50%", "75%", "100%"), aesthetics = c("colour", "fill")) + 
  ...
  ggtitle("HIV incidence \n(per 1000; 15-49 years)")
suppfig6a
ggsave("unaids_figures/art_incr_test_red.png", plot = suppfig6a, device = "png", units = "cm",
       height = 13, width = 13)
```
...
suppfig6b <- art_change_summary %>% 
  ...
  theme(axis.text = element_text(size = 11), 
        ...
        legend.title = element_text(size = 11)) +
  scale_y_continuous("HIV prevalence (%)", labels =(function(l) {round(l*1e2,1)}),expand = c(0,0), limits = c(0,0.2)) +
  scale_fill_manual(name = "Treatment\nretention", breaks = c("baseline", "0"), values = c("baseline" = "black", "0" = "#e41a1c"), labels= c("Baseline", "Decreased"), aesthetics = c("colour", "fill")) +
 ggtitle("HIV prevalence\n(15-49 years)")
suppfig6b
...
suppfig6c <- art_change_summary %>% 
  ...
  theme(axis.text = element_text(size = 11), 
        ...
        aspect.ratio=1, 
        legend.title = element_text(size = 11)) +
  scale_y_continuous("ART coverage (%) \n(15+ years)", labels =(function(l) {round(l*1e2,1)}), expand = c(0,0)) +
  scale_fill_manual(name = "Treatment\nretention", breaks = c("baseline", "0"), values = c("baseline" = "black", "0" = "#e41a1c"), labels= c("Baseline", "Decreased"), aesthetics = c("colour", "fill")) +
  ggtitle("ART coverage\n(15+ years)")
suppfig6c
...
suppfig6d <- art_change_summary %>% 
  ...
  theme(axis.text = element_text(size = 11), 
        ...
        legend.title = element_text(size = 11)) +
  scale_y_continuous("HIV tests per 100", labels =(function(l) {round(l*1e2,1)}), expand = c(0,0)) +
  scale_fill_manual(name = "Treatment\nretention", breaks = c("baseline", "0", "50"), values = c("baseline" = "black", `0` = "#e41a1c", `50` = "#377eb8"), labels= c("Baseline", "0%"), aesthetics = c("colour", "fill")) +
...
suppfig6 <- annotate_figure(suppfig6, top = text_grob("Increased ART interruption rate from 14.9% to 29.2%", 
               color = "black", face = "bold", size = 14))
ggsave(filename = "unaids_figures/suppfig6.pdf", plot = suppfig6, device = "pdf", width = 20, units = "cm")
ggsave(filename = "unaids_figures/suppfig6.png", plot = suppfig6, device = "png", width = 20, units = "cm")
suppfig6
```
...
suppfig7a <- condom_change_summary %>% 
  ...
  theme(axis.text = element_text(size = 11), 
        ...
        legend.title = element_text(size = 11)) +
  scale_y_continuous("HIV incidence", labels =(function(l) {round(l*1e3,1)}), breaks = c(0, 0.001, 0.002, 0.003, 0.004, 0.005, 0.006, 0.007), expand = c(0,0)) +
  scale_fill_manual(name = "Condom usage", breaks = c("baseline", "0"), values = c("baseline" = "black", "0" = "#377eb8"), labels= c("Baseline", "Increased"), aesthetics = c("colour", "fill")) +
...
suppfig7 <- annotate_figure(suppfig7, top = text_grob("Increased condom usage in 2035 from 33.9% to 45.9%", 
               color = "black", face = "bold", size = 14))
ggsave(filename = "unaids_figures/suppfig7.pdf", plot = suppfig7, device = "pdf", width = 20, units = "cm")
ggsave(filename = "unaids_figures/suppfig7.png", plot = suppfig7, device = "png", width = 20, units = "cm")
```
...
suppfig8a <- condom_change_summary %>% 
  ...
  ggplot(aes(year, mean, group = test_reduction)) +
  geom_ribbon(aes(ymin = lower_CI, ymax = upper_CI, fill = test_reduction), alpha = 0.10, show.legend = F) +
  geom_line(aes(colour = test_reduction, lty = scenario), show.legend = F) +
  geom_line(data = filter(condom_change_summary, 
              scenario == "baseline",
              pitc_reduction_year == 2025,
              indicator == "HIVinc15to49",
  ...
  scale_linetype_manual(values = c("baseline" = "dashed", "intervention" = "solid"), 
                        name = "Condom\nusage\nprobability",
                        breaks = c("baseline", "intervention"),
                        labels = c("Status quo", "Reduced")) + ggtitle("HIV incidence\n(per 1000; 15-49 years)")
suppfig8a

ggsave("unaids_figures/condom_inc_decr.png", device = "png", units = "cm", height = 13, width = 15)
```
...
suppfig9a <- art_change_summary %>% 
  filter(scenario == "intervention", 
         ...
         year >= 2020, 
         test_reduction == 0,
         art_coverage_2035 %in% c(0.499, 0.916)
         ) %>%
  ...
  geom_line(aes(colour = art_coverage_2035), show.legend = F) +
  ...
  scale_fill_manual(name = "ART coverage\nin 2035", 
                    breaks = c(0.499, 0.767, 0.916), 
                    values = c(`0.499` = "#e41a1c", `0.767` = "black", `0.916` = "#377eb8"), 
                    labels = c("50%", "77% (baseline)", "92%"), 
                    aesthetics = c("colour", "fill"))
suppfig9a
ggsave(plot = suppfig9a, filename = "unaids_figures/art_change_inc.png", height = 13, width = 15, units = "cm", device = "png")
```
...
suppfig9c <- art_change_summary %>% 
  ...
  theme(axis.text = element_text(size = 16), 
        ...
        legend.title = element_text(size = 11)) +
  ggtitle("ART coverage (%)\n(15+ years)") +
  scale_y_continuous("", labels =(function(l) {round(l*1e2,1)}), expand = c(0,0), limits = c(0,1)) + 
  scale_fill_manual(name = "ART coverage\nin 2035", 
                    breaks = c(0.499, 0.767, 0.916), 
                    values = c(`0.499` = "#e41a1c", `0.767` = "black", `0.916` = "#377eb8"), 
                    labels = c("50%", "77% (baseline)", "92%"), 
                    aesthetics = c("colour", "fill"))
ggsave(plot = suppfig9c, filename = "unaids_figures/art_covereage_change.png", height = 12.57, width = 14.96, units = "cm", device = "png")
...
suppfig9 <- annotate_figure(suppfig9, top = text_grob("ART interruption rate varied", 
               color = "black", face = "bold", size = 14))
ggsave(filename = "unaids_figures/suppfig9.pdf", plot = suppfig9, device = "pdf", width = 20, units = "cm")
ggsave(filename = "unaids_figures/suppfig9.png", plot = suppfig9, device = "png", width = 20, units = "cm")
suppfig9
```
...
suppfig10a <- condom_change_summary %>% 
  filter(scenario == "intervention", 
         ...
         year >= 2020, 
         test_reduction == 0,
         overall_condom_usage %in% c(14.1, 58.2)
         ) %>% 
  ...
  geom_line(aes(colour = overall_condom_usage), show.legend = F) +
  geom_hline(aes(yintercept = 0.001), lty = "dotted") +
  geom_line(data = filter(condom_change_summary,scenario == "intervention",
                   ...
                   aes(year, mean, group = overall_condom_usage), linewidth = 0.75, show.legend = F) +
  ...
  scale_y_continuous("", labels =(function(l) {round(l*1e3,1)}),expand = c(0,0)) + 
    scale_fill_manual(name = "Condom usage \nin 2035", 
                    breaks = c(14.1,33.9, 58.2), 
                    values = c(`14.1` = "#e41a1c", `58.2` = "#377eb8", `33.9` = "black"), 
                    labels= c("14%","34% (Baseline)","58%"), 
                    aesthetics = c("colour", "fill")) 
suppfig10a
ggsave("unaids_figures/suppfig10a.png", suppfig10a, device = "png", units = "cm", height = 13, width = 15)
```
...
suppfig10d <- condom_change_summary %>% 
  ...
  theme(axis.text = element_text(size = 16), 
        ...
        aspect.ratio=1, 
        legend.title = element_text(size = 11)) +
  ggtitle("Sex acts protected by condoms (%)\n (15+ years)") +
  ...
  scale_fill_manual(name = "Condom usage \nin 2035", 
                    ...
                    labels= c("14%","34% (Baseline)","58%"), 
                    aesthetics = c("colour", "fill")) 
ggsave(plot = suppfig10d, filename = "unaids_figures/condom_change.png", width = 15, height = 13, units = "cm", device = "png")
```
...
suppfig10 <- annotate_figure(suppfig10, top = text_grob("Probabilty of condom usage varied", 
               color = "black", face = "bold", size = 14))
ggsave(filename = "unaids_figures/suppfig10.pdf", plot = suppfig10, device = "pdf", width = 20, units = "cm")
ggsave(filename = "unaids_figures/suppfig10.png", plot = suppfig10, device = "png", width = 20, units = "cm")
```
...
```{r total_adult_hiv_tests, echo = false}
suppfig25 <- test_reduction_only %>% mutate(test_reduction = as.factor(test_reduction)) %>% 
  filter(scenario == "intervention", 
         pitc_reduction_year == 2025, 
         indicator == "TotalHIVtests",
         year >= 2020, 
         test_reduction %in% c(0, 100)) %>% 
  mutate(test_reduction = factor(as.character(test_reduction), labels = c("100", "0"), levels = c("100", "0"))) %>% 
  ggplot(aes(year, mean, group = test_reduction, fill = test_reduction)) +
  geom_ribbon(aes(ymin = lower_CI, ymax = upper_CI, fill = test_reduction), alpha = 0.10, show.legend = F) +
  geom_line(aes(colour = test_reduction), show.legend = T) +
  scale_x_continuous("", expand = c(0, 0)) +
  ...
  theme(axis.text = element_text(size = 11), 
        ...
        legend.title = element_text(size = 11)) +
  scale_y_continuous("", labels =(function(l) {round(l/1e6,1)}),expand = c(0, 0)) +
  scale_fill_manual(name = "General\nHTS\nreduction", breaks = c(0, 100), values = c(`0` = "black", `100` = "#e41a1c"), labels= c("None", "100%"), aesthetics = c("colour", "fill")) + 
  ggtitle("Adult HIV tests\n(millions; 15+ years)") 
suppfig25

suppfig25_combined<- ggarrange(fig1d, suppfig25, nrow = 1, ncol = 2, legend = "right", common.legend = TRUE) 
ggsave(filename = "figures/total_tests.png", plot = suppfig25_combined, device = "png", units = "cm", width = 18.92)
```
